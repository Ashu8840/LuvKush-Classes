...
export type Exam = {
  ...
  isPublished?: boolean;
  isTimed?: boolean;
  timingStatus?: "upcoming" | "live" | "ended";
...
export type Fee = {
  ...
  status: string;
  remarks?: string;
  student?: string | { _id: string; name: string; email?: string; phone?: string };
};
...
export type StudentReport = {
  ...
  shorthand: {
    ...
    avgWpm: number;
    avgAccuracy: number;
    recent: { _id: string; accuracy: number; wpm: number; dictation?: { title: string }; createdAt: string }[];
  };
  typing: {
    totalSessions: number;
    avgWpm: number;
    avgAccuracy: number;
    recent: { _id: string; wpm: number; accuracy: number; practicedAt: string; passage?: { title: string } }[];
  };
  attendance: {
    summary: AttendanceStats & { percent: number; totalMarked: number };
    recent: AttendanceRecord[];
  };
};

export type AttendanceMarkList = {
...
export type Certificate = {
  ...
  rank?: number;
  course?: { name: string };
  program?: { _id: string; title: string; description?: string };
...
async function uploadFile(file: File, folder = "materials") {
  const token = getToken();
...
export const api = {
  ...
  getStudentReport: (studentId: string) =>
    request<StudentReport>(`/teacher/students/${studentId}/report`),
  getAttendanceMarkList: (batchId?: string) =>
    request<AttendanceMarkList>(`/attendance/mark-list${batchId ? `?batchId=${batchId}` : ""}`),
  ...
  exportFees: (fromDate: string, toDate: string) =>
    downloadCsv(`/fees/export?fromDate=${fromDate}&toDate=${toDate}`, `fees-${fromDate}-to-${toDate}.csv`),
  exportPayments: (fromDate: string, toDate: string, status?: string) => {
    const q = new URLSearchParams({ fromDate, toDate });
    if (status) q.set("status", status);
    return downloadCsv(`/fees/payments-export?${q}`, `approvals-${fromDate}-to-${toDate}.csv`);
  },
  ...
  exportAttendance: (params: string) =>
    downloadCsv(`/attendance/export?${params}`, `attendance-report.csv`),
  ...
  verifyCertificate: (id: string) =>
    request<Certificate>(`/certificates/verify/${id}`),
  getCertificateRenderUrl: (certificateId: string) => {
    const token = getToken();
    const base = `${API_URL}/certificates/render/${certificateId}`;
    return token ? `${base}?token=${encodeURIComponent(token)}` : base;
  },

  getCertificationPrograms: () => request<CertificationProgram[]>("/certifications"),
  getCertificationProgram: (id: string) => request<CertificationProgram>(`/certifications/${id}`),
  createCertificationProgram: (data: Record<string, unknown>) =>
    request<CertificationProgram>("/certifications", { method: "POST", body: JSON.stringify(data) }),
  updateCertificationProgram: (id: string, data: Record<string, unknown>) =>
    request<CertificationProgram>(`/certifications/${id}`, { method: "PUT", body: JSON.stringify(data) }),
  ...
  startCertificationExam: (programId: string) =>
    request<CertificationExam>(`/certifications/${programId}/start-exam`, { method: "POST" }),
  submitCertificationExam: (programId: string, attemptId: string, answers: { questionIndex: number; answer: string }[]) =>
    request<CertificationResult>(`/certifications/${programId}/submit-exam`, {
      method: "POST",
  ...
  // Typing Library
  getTypingPassages: (params?: { language?: string; difficulty?: string; category?: string }) => {
  ...
  getTypingPassage: (id: string) => request<TypingPassage>(`/typing/passages/${id}`),
  ...
  generateTypingPassage: (data: {
    ...
    request<{ title: string; content: string; wordCount: number; language: string; difficulty: string }>(
      "/typing/passages/generate",
  ...
  uploadFile,
  uploadAudio: (file: File) => uploadFile(file, "dictations"),
  uploadPaymentProof: (file: File) => uploadFile(file, "payment-proofs"),
  uploadAvatar: (file: File) => uploadFile(file, "avatars"),
  uploadAnnouncementImage: (file: File) => uploadFile(file, "announcements"),
};